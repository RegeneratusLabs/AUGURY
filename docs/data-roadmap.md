...
| Image gallery | `data/vision/images/` (15GB, 111,320 imgs) | ✅ sidecars per species; **local-only** build source (removed from HF 2026-08-18) |
...
- **Portable image paths**: builder emits relative paths (was: absolute, broke
  any non-local run).
- **Gallery kept local-only** (removed from HF 2026-08-18 — raw images are a
  build-time source, never read at runtime; the runtime artifact is the compact
  FAISS index; see `docs/gallery-phone-discussion.md`).
